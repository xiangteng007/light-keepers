...
| **Phase M 前置** | ✅ 紙上作業就緒 | M.0 compose env 修復；M.1 NAS 生產棧（`infra/nas/`：五服務 compose＋備份 job＋還原演練腳本＋部署手冊）；M.2 本地 LLM provider（hybrid fallback＋對測工具）；M.3 搬遷腳本（DB/GCS，dry-run 驗證）；🔄 M.3b storage 抽象層接管執行中（**搬遷阻斷項**：三個檔案 service 直用 GCS SDK）；⏸ 實際搬遷等 D15 租約日＋owner 架 Ollama/Tunnel |
