...
| **Phase 3** | 🔄 進行中 | ✅ 3.1 API client 收斂方案＋ESLint 防護欄（148 個裸 fetch 呼叫點分類、paths.ts）、✅ 3.3 mock 頁處置（mission-command/triage 換掛真版）、✅ 3.4 離線層收斂（**修復三個 P0 資料遺失 bug**：唯一生效的離線同步是假實作、失敗當成功刪佇列、清快取誤刪未送資料；單一 Dexie outbox＋27 測試）、✅ 3.5 DTO 化（37 DTO＋堵伺服器欄位覆寫）、✅ 3.6 個資遮罩 interceptor＋mood-tracker IDOR 修補、🔄 3.2 API 批次遷移執行中 |
