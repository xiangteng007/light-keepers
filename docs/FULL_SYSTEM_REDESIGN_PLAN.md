...
## 執行進度（2026-08-04 對帳更新；任務編號 O/N/F 見 `MODEL_TASK_ASSIGNMENTS.md`）
...
| **Phase 1** | 🔄 進行中 | ✅ 1.1 SYNC_TABLES production 硬禁用（cloudbuild 源頭同步清除——**證實生產原本開著 synchronize**）、✅ 1.3 重複 entity 收斂、✅ 1.4 授權盤點（實際 43 裸 controller/361 端點）、✅ 1.5a+1.5b 共 55 controller/383 端點定級補 guard、🔄 1.6 guard 收斂收尾（O22 READY）、🟢 1.2 baseline migration **READY**（D20 定案＝資料不可取回→重新開站，**窗口約束解除**；任務 O1，範圍含損壞 migration＋稽核靜默失敗） |
...
| **Phase 4** | ✅ 可派項完成 | 4.1 單租戶降級（D9）＋限流收緊（**發現既有 18 個 @Throttle 全部因命名不匹配靜默失效**，登入實跑 100/min，已修）、4.2 刪除 35 個 stub 模組（D10；17,848 行、依賴圖攔下 line-notify/realtime-chat 兩個誤刪、fire-119 凍結）、4.5 console→Logger（45 處）＋no-console gate＋strictFunctionTypes；🟢 4.3 Map 狀態遷移（O13 READY）、🟢 4.4 i18n（D11 已拍板→基建 O14／批次 N6） |
| 待 Phase 4 處理（Phase 3 發現） | 📌 | ① `international-standards`/`reporting-engine`/`scalability` 三模組**從未註冊進 app.module**（1.5 的 guard 與 3.5 的 DTO 保護的是 404 端點）；註冊前須先解 `reporting-engine` 與 `reports` 的 `@Controller('reports')` 前綴衝突 ② 23 個 controller 雙前綴（`/api/v1/api/*`）完整清單在 3.3 回報 ③ ✅ `ReportSchedulePage` 404 已修（2026-08-04 F10：路徑/動詞對齊＋後端 history 路由順序修正；殘留 DTO 形狀對齊另列）④ ✅ 3 個死檔 service 已刪（2026-08-04 F11） |
| **Phase M 前置** | ✅ 紙上作業就緒 | M.0 compose env 修復；M.1 NAS 生產棧（`infra/nas/`：五服務 compose＋備份 job＋還原演練腳本＋部署手冊）；M.2 本地 LLM provider（hybrid fallback＋對測工具）；M.3 搬遷腳本（DB/GCS，dry-run 驗證）；✅ M.3b storage 抽象層接管（三 service 全遷、per-feature bucket 綁定、local 簽章 URL 向前相容方案；**已知缺口**：local 模式的 write 直傳 URL 無落點（nginx 405），影響 field-reports 附件直傳，列 README §8 後續）；⏹ 實際搬遷已無：D20 定案資料不可取回→**改重新開站路徑**（搬遷腳本退役；`EXECUTION_PLAN_RELAUNCH.md` A1 baseline→B1 起棧）；`LLM_PROVIDER=hybrid` 預設值已入 compose（2026-08-04 F1） |
...
| **Phase R（FABLE 重設計）** | 🔄 | ✅ R1 完成（DESIGN_LANGUAGE.md、雙模式 Shell、權限單一來源、24 redirect、5 權限修正、14 假 widget 頁改 placeholder、132/132 tests）；✅ R2 完成（R2a 態勢類：COP 牆＋角色分層 Dashboard；R2b 行動類：30 秒通報流＋真離線佇列、MapPage 拆分、應變工作檯；157/157 tests）；✅ R3 完成（R3a 14 頁＋R3b 63 頁＝77 頁依 archetype 重建；順修多個既有壞頁）；✅ R4 完成（78 張三態×雙尺寸截圖基準、axe AA 39/39 零違規——修掉兩個系統性 dark 模式對比根因、硬編色防復發 gate）。**Phase R 全部完成**；✅ 孤兒圈選已回（2026-08-02：全刪，見 PAGE_REMOVAL_PROPOSAL 圈選結果）。⚠ **2026-08-02 owner 實機檢視後否決 Phase R 視覺成果**（未貫徹 2026-01-05 戰術規範：平時模式成了白底通用後台）→ 立案 **Phase R5／FE-8 戰術化全面重構**，任務 prompt 已備妥於 `docs/FRONTEND_TACTICAL_REBUILD_PROMPT.md`（以 `lightkeepers_tactical_uiux_spec_utf8_bundle/` 為美學權威；T1 Shell+Dashboard 為 owner 閘門，避免重蹈全部做完才否決）。結構層（雙模式 Shell／權限單一來源／archetype／離線）保留沿用。✅ **R5/FE-8 戰術化重構已收官（2026-08-04）**：B3c token/排印/旗艦頁/圖文系統 84 icon/axe AA 零違規/48 張基準，孤兒 79 檔刪除、App.css 一階考古 −1,658 行、emoji 債 115 檔清償、T6 收官（明細見 `MODEL_TASK_ASSIGNMENTS.md`「已完成」表；殘餘小項歸 N8/N9/N10/F12） |

| **P0 補強（2026-08-02）** | ✅ | 以「掛了 UnifiedRolesGuard 卻沒有任何 `@RequiredLevel`＝guard fail-open」重掃，補完 **14 個殘餘 controller** 的定級（含 tactical-maps／triage／staff-security／donor-reporting；紀錄見 `docs/audit/AUTHZ_LEVELS_APPLIED_C.md`）；修 `req.user.userId` 執行期恆為 undefined（triage 的評估者/改判者/處置者留痕與 heartbeat break-glass 操作者一直沒被記錄）；新增 `security-invariants.spec.ts` 三項阻擋式回歸守衛（定級覆蓋率／`@Body() any`／外洩密鑰）；CI 補後端 lint 閘門、`npm test --coverage`（**coverageThreshold 先前在 CI 從未生效**）、dist 不得含 `devModeUser`、production 部署設定不得開 `SYNC_TABLES` 且必須掛 `JWT_SECRET`。**輸入驗證**：真正的洞不是 `@Body() any`（已清零）而是**行內型別**——編譯後沒有 metatype，ValidationPipe 整段跳過。本輪補完 auth/2FA 23 端點（含修好兩個真的壞掉的端點：`GET /auth/check-email-verification` 會回傳任意帳號驗證狀態、`/auth/2fa/verify` 系列在正式環境恆 400）＋作戰情資/派遣/個資 24 端點（SITREP/IAP/AAR/map-dispatch/簽到座標/心理健康），並加「行內型別不得復發」棘輪（基準線 111）。**留給 owner**：C-1 財務端點 L3 是否符合實務、C-3 auth 38 端點的公開介面清單、C-5 前端 2FA verify 少送 secret、C-6 其餘 106 處分批補的排序 |
...
**等 Owner 的動作（2026-08-04 對帳）**：①憑證輪換＋git 歷史清理確認（R13 鎖鏈，解鎖 push/CI）②D2 路由圈選 ③D-MCI 六決策 ④UPS＋4G/5G 路由器採購 ⑤工作站 Ollama 架設（S2.2）⑥LINE/LIFF/Google callback 重登記（B3）⑦UptimeRobot、B2/R2 帳號開通 ⑧開站公告發布＋PWA 重裝動員（草稿/教學已備，見 S7.1/S7.3）。~~D7/D15 窗口~~ 與 D12–D14/D20–D27 已全數定案（見 SELF_HOSTED_MIGRATION_PLAN 決策表）。
...
| D11 | 多語系是真需求嗎？（現況 0/137 頁使用，13 語系檔 10 個未註冊） | ✅ **已拍板（2026-08-02）：要多語**。落地即 4.4/FE-6 走「真需求」路線：從版型元件開始逐層接 `useTranslation`，語系範圍與優先順序（建議 zh-TW 主＋en，其餘視服務對象再加）由執行工作項首輪確認後回寫 | 工作量≈從零開始（137 頁）；建議排在孤兒刪除之後（少 36 檔＝少翻一批死頁） |
