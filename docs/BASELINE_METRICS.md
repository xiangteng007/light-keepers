...
> **產出日期**: 2026-07-31（後端模組計數於 2026-08-01 更新，見下）
> **最後更新**: 2026-08-01 — 工作項 4.2 / BE-4 刪除 35 個 stub 模組後重新計數（§一 全部指標）
...
| **本文件（實測）** | **84**（目錄）/ **103**（`.module.ts` 檔） | **137** | **2026-08-01** |
...
| # | 指標 | 定義 | 實測值 | 計數指令 |
|---|------|------|:------:|----------|
| 1 | 後端模組**目錄**數 | `backend/src/modules/` 下第一層子目錄數 | **84** | `find backend/src/modules -maxdepth 1 -mindepth 1 -type d \| wc -l` |
| 2 | `*.module.ts` 檔案數（全部） | `backend/src` 樹下所有 `*.module.ts`，含 `modules/`、`core/`、`common/`、`health/`、根目錄 | **103** | `find backend/src -name "*.module.ts" -not -path "*/node_modules/*" \| wc -l` |
| 2a | ├─ `src/modules/**/*.module.ts` | | 84 | `find backend/src/modules -name "*.module.ts" \| wc -l` |
...
| 3 | `app.module.ts` `imports:` 陣列中的 `*Module` 識別字（不含被 `//` 註解吃掉的部分） | 對 `imports: [...]` 區塊（第 167–346 行）逐行剝除 `//` 之後文字，再抓 `Module` 結尾識別字 | **83** | `awk 'NR==167,NR==346' backend/src/app.module.ts \| sed 's#//.*##' \| grep -oE '[A-Z][A-Za-z0-9]*Module' \| sort -u \| wc -l` |
| 3a | 同區塊內**全部**出現過的 `*Module` 識別字（含被註解吃掉的、含 `// REMOVED:` 標記的） | 不剝除註解 | 102 | `awk 'NR==167,NR==346' backend/src/app.module.ts \| grep -oE '[A-Z][A-Za-z0-9]*Module' \| sort -u \| wc -l` |
| 4 | Controller 檔案數 | `*.controller.ts`，排除 `*.spec.ts` | **121** | `find backend/src -name "*.controller.ts" -not -name "*.spec.ts" \| wc -l` |
| 5 | Service 檔案數 | `*.service.ts`，排除 `*.spec.ts` | **220** | `find backend/src -name "*.service.ts" -not -name "*.spec.ts" \| wc -l` |
| 6 | Entity 檔案數 | `*.entity.ts` | **105** | `find backend/src -name "*.entity.ts" \| wc -l` |
| 7 | 單元/整合測試（spec）檔案數 | `*.spec.ts`（`backend/src` 樹下） | **325** | `find backend/src -name "*.spec.ts" \| wc -l` |
...
### 備註（2026-08-01 工作項 4.2 / BE-4 的計數變動）

BE-4 依 `docs/FULL_SYSTEM_REDESIGN_PLAN.md` 決策 D10 刪除 35 個純 stub 模組，判定過程與逐模組證據見
`docs/architecture/STUB_DELETION_REPORT.md`。相對於刪除前基準（commit `a95d424`）的變動：

| 指標 | a95d424 | 現值 | 差 | 說明 |
|------|:-------:|:----:|:--:|------|
| 模組目錄數 | 119 | 84 | −35 | 刪除的 35 個模組 |
| `*.module.ts`（全部） | 135 | 103 | −32 | 35 − 3（`iot/`、`permissions/`、`supply-chain/` 原本就沒有 `.module.ts`） |
| Controller | 122 | 121 | −1 | 僅 `rag-knowledge.controller.ts`（其餘 34 個模組本來就沒有 controller） |
| Service | 259 | 220 | −39 | |
| Entity | 105 | 105 | 0 | **刪除集合完全沒有 entity**——這正是判定口徑的核心條件 |
| spec | 366 | 325 | −41 | 40 支隨模組移除 ＋ `src/test/i18n-api.service.spec.ts` |

`npx jest` 由 366 suites / 3,613 tests 降為 **325 suites / 3,159 tests，全綠、零失敗**。測試總數下降屬預期
（spec 隨模組一併移除），非測試退化。

模組目錄數與 §2a 的檔案數現已相等（84 = 84）：先前造成落差的 3 個無 `.module.ts` 目錄已刪除，
而 `shared/` 目前也只剩 `shared-auth.module.ts` 一個 module 檔（`shared-jwt.module.ts` 已於先前工作項移除）。

`app.module.ts` 的 `imports:` 陣列仍存在「模組名被 `//` 註解吃掉」的已知現象（見
`docs/FULL_SYSTEM_REDESIGN_PLAN.md` §1.5-3、工作項 E.3）：陣列文字中共出現 102 個相異 `*Module`
識別字，其中僅 83 個在註解剝除後仍存在、於編譯期真正屬於 `imports:` 成員。差額包含（a）故意寫的
`// REMOVED: XxxModule` 說明性註解，以及（b）因換行被壓縮而實際未被註冊的模組。本文件僅提供可重跑的
計數方法，逐項清單以該計畫文件的分析為準。

> ⚠️ §3 / §3a 的 `awk` 行號區間會隨 `app.module.ts` 編輯而失效。重跑前請先以
> `grep -n 'imports: \[' backend/src/app.module.ts` 與其對應的 `],` 確認當前區間。
...
| 指標 | 舊文件數字 | 本文件實測 | 差異原因 |
|------|:----------:|:----------:|----------|
| 後端模組 | 175（`00-system-inventory.md`、`SYSTEM_OPTIMIZATION_PLAN.md`） | 84 目錄 / 103 `.module.ts` 檔 | 舊數字疑為人工盤點或含已刪除/合併模組，口徑未定義是否含 `core`/`common`/`health`；本欄 2026-08-01 因 BE-4 刪除 35 個 stub 模組再降（原 119 目錄 / 135 檔） |
