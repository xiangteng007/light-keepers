...
export default function ReportSchedulePage() {
    ...
    const formatFrequency = (schedule: ReportSchedule) => {
        ...
        if (schedule.frequency === 'weekly' && schedule.dayOfWeek !== undefined) {
            const days = ['�?, '一', '�?, '�?, '�?, '�?, '�?];
            detail = ` �?{days[schedule.dayOfWeek]}`;
            ...
            <div className="schedules-grid">
                {loading ? (
                    ...
                    <div className="empty">
                        <Calendar size={48} />
                        <p>尚無報表排程</p>
                        <button onClick={() => setShowCreateModal(true)}>建立第一個排�?/button>
                    </div>
                ) : (
