...
export default function ProfilePage() {
    ...
    const handlePreferenceChange = async (key: keyof typeof preferences, value: boolean) => {
        ...
        try {
            ...
            if (response.ok) {
                setMessage({ type: 'success', text: '通知偏好已更�? });
    ...
    const handleChangePassword = async () => {
        ...
        if (passwordData.newPassword.length < 6) {
            setMessage({ type: 'error', text: '新密碼至少需�?6 個字�? });
            return;
        }
    ...
    const handleSetPassword = async () => {
        ...
        if (passwordData.newPassword.length < 6) {
            setMessage({ type: 'error', text: '密碼至少需�?6 個字�? });
            return;
        }
    ...
    const handleVolunteerSubmit = async () => {
        if (!volunteerForm.name || !volunteerForm.phone || !volunteerForm.region || !volunteerForm.emergencyContact || !volunteerForm.emergencyPhone) {
            setMessage({ type: 'error', text: '請填寫必填欄位：姓名、電話、所在地區、緊急聯絡人、緊急聯絡電�? });
            return;
        }
                            ...
                            <div className="profile-form">
                                <div className="profile-form-group">
                                    ...
                                    {isEditing ? (
                                        ...
                                        <div className="profile-value">{user?.displayName || '未設�?}</div>
                                    )}
                                </div>

                                <div className="profile-form-group">
                                    <label><Mail size={16} /> 電子郵件</label>
                                    <div className="profile-value">{user?.email || '未設�?}</div>
                                </div>
                                ...
                                <div className="security-item">
                                    <div className="security-item-info">
                                        {hasPassword ? (
                                            ...
                                            <>
                                                <h4>設定密碼</h4>
                                                <p>您透過 LINE/Google 登入，尚未設定密碼。設定後可使�?Email/密碼登入</p>
                                            </>
                                        )}
                                    </div>
                                    <button
                                    ...
                                    >
                                        {hasPassword === null ? '載入�?..' : hasPassword ? '變更密碼' : '設定密碼'}
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="profile-section volunteer-register-section">
                            ...
                            {volunteerSubmitted ? (
                                ...
                                <div className="volunteer-form">
                                    ...
                                    <div className="form-section">
                                        <label className="form-label">專長技�?/label>
