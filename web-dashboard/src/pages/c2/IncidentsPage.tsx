...
const STATUS_INFO: Record<string, { label: string; color: string; icon: React.ElementType; badgeVariant: BadgeProps['variant'] }> = {
    reported: { label: '已通報', color: '#6b7280', icon: ClockIcon, badgeVariant: 'default' },
    confirmed: { label: '已確認', color: 'var(--color-info, #3b82f6)', icon: WarningIcon, badgeVariant: 'info' },
    active: { label: '進行中', color: 'var(--color-warning, #f59e0b)', icon: WarningIcon, badgeVariant: 'warning' },
    in_progress: { label: '處理中', color: 'var(--color-warning, #f59e0b)', icon: WarningIcon, badgeVariant: 'warning' },
    resolved: { label: '已解決', color: 'var(--color-success, #22c55e)', icon: CheckIcon, badgeVariant: 'success' },
    closed: { label: '已結案', color: 'var(--color-success-dark, #10b981)', icon: CheckIcon, badgeVariant: 'success' },
};
                ...
                <div className="incidents-toolbar">
                    ...
                    <div className="page-actions">
                        <button
                            ...
                            title="重新整理"
                        >
                            <SyncIcon size={16} className={loading ? 'spin' : ''} aria-hidden="true" />
                        </button>
                        <Button variant="primary" icon={<PlusIcon size={16} aria-hidden="true" />}>
                            新增事件
                        </Button>
                    </div>
                </div>
                                ...
                                <div key={incident.id} className="incident-card">
                                    ...
                                    <div className="incident-content">
                                        ...
                                        <div className="incident-meta">
                                            {incident.location && <span><LocationIcon size={12} aria-hidden="true" /> {incident.location}</span>}
                                            <span className="tabular-nums"><ClockIcon size={12} aria-hidden="true" /> {incident.reportedAt || (incident.createdAt ? new Date(incident.createdAt).toLocaleString('zh-TW') : '-')}</span>
                                            {incident.assignedTeams != null && <span className="tabular-nums"><TeamsIcon size={12} aria-hidden="true" /> {incident.assignedTeams} 小隊</span>}
                                        </div>
                                    </div>
                                </div>
                            );
