...
// 台灣縣市中心座標 (用於無 GPS 座標時的預設位置)
const TAIWAN_COUNTY_COORDS: Record<string, [number, number]> = {
    // 六都
    '台北': [25.0330, 121.5654],
    '臺北': [25.0330, 121.5654],
    '新北': [25.0120, 121.4650],
...
export class NcdrAlertsService {
    ...
    parseAtomEntry(entry: any, alertTypeId: number): Partial<NcdrAlert> | null {
        try {
            ...
            // 對於台灣的警報，如果沒有座標，嘗試從標題提取縣市座標
            if (!latitude || !longitude || isNaN(latitude) || isNaN(longitude)) {
                // 1. 優先嘗試從標題中提取縣市名稱
                let coordsFound = false;
                for (const [countyName, coords] of Object.entries(TAIWAN_COUNTY_COORDS)) {
                    if (title.includes(countyName)) {
                        latitude = coords[0];
                        longitude = coords[1];
                        coordsFound = true;
                        break;
                ...
                // 2. 如果標題中沒有縣市，嘗試從描述中提取
                if (!coordsFound && summary) {
                    for (const [countyName, coords] of Object.entries(TAIWAN_COUNTY_COORDS)) {
                        if (String(summary).includes(countyName)) {
                            latitude = coords[0];
                            longitude = coords[1];
                            coordsFound = true;
                            break;
                ...
                if (!coordsFound) {
                    const defaultCoords: Record<number, [number, number]> = {
                        6: [23.9, 121.6],       // 地震 - 台灣東部海域
                        7: [23.5, 121.8],       // 海嘯 - 東海岸外海
                        5: [23.6978, 120.9605], // 颱風 - 台灣中心
                        10: [25.0330, 121.5654], // 降雨 - 台北
                        1051: [23.6978, 120.9605], // 雷雨 - 台灣中心
                        9: [23.8, 120.8],       // 土石流 - 中部山區
                        1087: [25.0330, 121.5654], // 火災 - 台北
                        34: [24.5, 121.0],       // 鐵路事故(臺鐵) - 中部鐵路線
                        32: [24.8, 121.0],       // 鐵路事故(高鐵) - 高鐵沿線
                        1060: [23.6978, 120.9605], // 低溫 - 台灣中心
                        2107: [23.6978, 120.9605], // 高溫 - 台灣中心
                        1061: [23.6978, 120.9605], // 強風 - 台灣中心
                        1062: [23.6978, 120.9605], // 濃霧 - 台灣中心
                        8: [25.0330, 121.5654],  // 淹水 - 台北
                        11: [23.6978, 120.9605], // 河川高水位 - 台灣中心
                        12: [23.6978, 120.9605], // 水庫放流 - 台灣中心
                    };
